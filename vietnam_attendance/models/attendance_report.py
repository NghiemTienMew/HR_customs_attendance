    ...
    def _select(self):
        return """
            SELECT
                ...
                e.job_id,
                e.user_id,
                DATE(a.check_in AT TIME ZONE 'UTC' AT TIME ZONE COALESCE(p.tz, 'UTC')) AS date,
                a.shift_id,
                a.check_in,
                a.check_out,
                a.worked_hours,
                ...
                a.check_out_method,
                a.company_id,
                EXTRACT(DOW FROM DATE(a.check_in AT TIME ZONE 'UTC' AT TIME ZONE COALESCE(p.tz, 'UTC'))) IN (0, 6) AS is_weekend,
                EXISTS (
                    SELECT 1 FROM vietnam_attendance_holiday h 
                    WHERE h.date = DATE(a.check_in AT TIME ZONE 'UTC' AT TIME ZONE COALESCE(p.tz, 'UTC'))
