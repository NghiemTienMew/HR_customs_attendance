...
class HrEmployee(models.Model):
    ...
    # Cập nhật phụ thuộc vào overtime_status
    total_overtime = fields.Float(string='Tổng thời gian tăng ca', compute='_compute_total_overtime')
    ...
    # Cập nhật phương thức để phụ thuộc vào overtime_status
    @api.depends('attendance_ids.overtime_status')
    def _compute_total_overtime(self):
        for employee in self:
            # Tính tổng thời gian tăng ca từ model vietnam.attendance.overtime
            overtimes = self.env['vietnam.attendance.overtime'].search([
                ('employee_id', '=', employee.id),
                ('state', '=', 'approved')  # Chỉ tính các bản ghi đã được phê duyệt
            ])
            
            # Lấy các bản ghi chấm công có trạng thái tăng ca đã được duyệt
            overtime_attendances = self.env['vietnam.attendance'].search([
                ('employee_id', '=', employee.id),
                ('attendance_type', '=', 'overtime'),
                ('overtime_status', '=', 'approved')
            ])
            
            # Tính tổng thời gian tăng ca từ cả hai nguồn
            overtime_hours_from_overtimes = sum(overtimes.mapped('duration'))
            overtime_hours_from_attendances = sum(overtime_attendances.mapped('worked_hours'))
            
            employee.total_overtime = overtime_hours_from_overtimes + overtime_hours_from_attendances
