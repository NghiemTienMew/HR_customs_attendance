...
class HrEmployee(models.Model):
    ...
    # Thêm trường overtime_ids để liên kết với Vietnam Attendance Overtime
    overtime_ids = fields.One2many('vietnam.attendance.overtime', 'employee_id', string='Tăng ca')
    
    # Thêm trường total_overtime với phụ thuộc đúng vào overtime_ids
    total_overtime = fields.Float(string='Tổng thời gian tăng ca', compute='_compute_total_overtime', store=True)
    ...
    # Sửa lại phương thức tính total_overtime chỉ phụ thuộc vào overtime_ids
    # Loại bỏ mọi phụ thuộc vào vietnam.attendance.overtime_status
    # CHỈ phụ thuộc vào overtime_ids.duration và overtime_ids.state
    @api.depends('overtime_ids.duration', 'overtime_ids.state')
    def _compute_total_overtime(self):
        for employee in self:
            # Tính tổng thời gian tăng ca trực tiếp từ overtime_ids
            approved_overtimes = employee.overtime_ids.filtered(lambda o: o.state == 'approved')
            employee.total_overtime = sum(approved_overtimes.mapped('duration'))
